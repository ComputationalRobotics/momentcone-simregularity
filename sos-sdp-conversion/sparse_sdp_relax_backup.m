function SDP = sparse_sdp_relax_backup(problem)
	...
	assert(length(I) == length(H) && length(H) == length(G) && length(G) == length(kappa),...
		'problem.vars, problem.equality, problem.inequality and problem.relaxation_order should have the same shape');
	p = length(I);
	...
	for i = 1 : p
		for j = 1 : length(G{i})
			g = G{i}(j);
			v = mymonomial(I{i}, kappa(i) - ceil(deg(g) / 2));
			K.s(end + 1) = length(v);
			for j = 1 : length(v)
				pop = [ pop; g * v(j) * v(1 : j) ];
			end
	...
	for i = 1 : p
		for j = 1 : length(H{i})
			h = H{i}(j);
			new_poly = h * monomials(I{i}, 0 : 2 * kappa(i) - deg(h));
		...
		for i = 1 : n
			for j = 1 : i
				...
				if vis(col)
					...
					vis(col) = true;
					map_sedumi(col, idx_sedumi_) = map_sedumi(col, idx_sedumi) + 0.5;
	...
	c = coef(idx_sdpt3, :) * map_sedumi;
	SDP.sedumi.c = c;
	equality_constraint_sedumi = coef(idx_sdpt3 + 1 : end, :) * map_sedumi;
	...
	C = cell(size(K.s));
	equality_constraint_sdpt3 = coef(idx_sdpt3 + 1 : end, :) * map_sdpt3;
